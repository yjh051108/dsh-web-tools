 ...
 *  与本插件（**本机 Windows 上的 Chrome · headless · 独立 profile**）**不共享 cookie/标签**，截图不能互证。
...
export const OTHER_BROWSER_HINT = '另一套浏览器 browser-harness（Windows Edge，CDP 9222，用户可见窗口）与本插件（本机 Windows 上的 Chrome，headless，独立 profile）不共享 cookie/登录态/标签：两套的截图不能互证；也别用 browser-harness 去驱动本插件 9339 的实例（会被本插件回收）。'
...
/* ══════════════════════════════════════════════════════════════════════════
 * ★★ v0.2.0「有头通道」（headed）—— 接上**人正看着的那个浏览器视图**
 *
 * 背景（用户定向：「完整给出我们的 electron 壳适配侧边栏的有头操控方案」）：
 *   本插件原先只有一条通道：**自己 spawn 一个 headless Chrome**（独立 profile、
 *   用户看不见、无登录态）。而 **electron 壳里已经有**一个可见的浏览器视图
 *   （`desktop/main.js` 的浏览器桥 + `dsh-agent-browser` 面板登记），
 *   两者此前**互不相通** ⇒ 本模块把壳那条路接进来。
 *
 * 接线点（逐字，`desktop/main.js`）：
 *   `:131  env.DSH_SHELL_BRIDGE_URL = \`http://127.0.0.1:${BRIDGE_PORT}\``
 *   `:368  默认 BRIDGE_PORT = 3091` · `:367 默认 TUNNEL_PORT = 3092`
 *   ⚠️ **该环境变量只在【壳启动的 dsh 进程】里存在** ——
 *      独立跑 `dsh web` 时它是空的 ⇒ **有头通道不可用**（**这一点必须明说，不许静默回落**）。
 *
 * 桥的两个端点（`main.js:387-424`）：
 *   `POST /open-window  body:{instanceId}` ⇒ 开一个桌面浏览器窗口
 *   `GET  /targets` ⇒ [{ id, url, title, instanceId, windowId,
 *                        cdpUrl: "ws://127.0.0.1:3092/?targetId=<id>" }]
 *   ⇒ ★ `cdpUrl` 可直接喂给现有的 `connectOnce()`（同一套 CDP over WebSocket）
 * ══════════════════════════════════════════════════════════════════════════ */

/** 壳桥地址（空 = 不在壳里跑 ⇒ 有头通道不可用）。 */
export function shellBridgeUrl() {
  return String(process.env.DSH_SHELL_BRIDGE_URL || '').replace(/\/+$/, '')
}

/** 读壳桥的 /targets（只读）。返回 { ok, url, targets, error }。 */
export function shellTargets(timeoutMs = 1500) {
  return new Promise((resolve) => {
    const base = shellBridgeUrl()
    if (!base) {
      return resolve({ ok: false, url: '', targets: [], error: 'DSH_SHELL_BRIDGE_URL 未设置（不在 electron 壳里）' })
    }
    const req = http.get(base + '/targets', (res) => {
      let d = ''
      res.on('data', (c) => (d += c))
      res.on('end', () => {
        try {
          const arr = JSON.parse(d)
          resolve({ ok: true, url: base, targets: Array.isArray(arr) ? arr : [] })
        } catch (e) {
          resolve({ ok: false, url: base, targets: [], error: 'targets 非 JSON: ' + String(e.message).slice(0, 80) })
        }
      })
    })
    req.on('error', (e) => resolve({ ok: false, url: base, targets: [], error: e.message }))
    req.setTimeout(timeoutMs, () => { try { req.destroy() } catch { } resolve({ ok: false, url: base, targets: [], error: 'timeout' }) })
  })
}
...
export function headedUnavailable(why) {
  const base = shellBridgeUrl()
  return {
    ok: false,
    channel: 'headed',
    error: '有头通道不可用：' + why,
    hint: base
      ? `壳桥地址 = ${base}；若 /targets 为空 ⇒ 壳里还没有打开任何浏览器视图（先用 dsh-agent-browser 开一个）。`
      : '未检测到 DSH_SHELL_BRIDGE_URL ⇒ 你现在不是在【桌面壳】里跑（例如是 `dsh web`）。' +
        '有头通道只在桌面壳（DSH-Desktop）下可用；★ 想要无头截图请显式传 channel:"headless"。',
  }
}
...
export function apply(ctx, config) {
  ...
  /* ── ★★ v0.2.0 有头通道（headed）：接壳里那个【人正看着】的视图 ──────────
   * 与无头的**三点不同**（这三条正是用户要的"完整说明"）：
   *   ① 连的是**壳桥给的 cdpUrl**（`ws://127.0.0.1:3092/?targetId=<id>`），
   *      **不是**自己 spawn 的 9339 实例 ⇒ **不 spawn、不杀进程**
    ...
    async execute() {
      ...
      try {
        ...
        return {
          ...
          // ★★ 两条通道各自的就绪性（用户/模型一眼看清"有头能不能用"）
          channels: {
            headless: { ready: true, what: '自己 spawn 的无头 Chrome（独立 profile、用户看不见、无登录态）', port },
            headed: {
              ready: !!(st.ok && tgt),
              bridgeUrl: sb || '',
