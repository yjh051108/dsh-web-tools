...
> **多视图同时开着时，它挑哪个？**（v0.2.1 起）
> **优先挑「属于你当前会话」的那个** —— 判据是 `/targets` 里的 `windowId`
> （= 那个视图所属的**会话 id**，见下）。
> ⚠️ **万一挑不到**（你这个会话还没开视图、或拿不到会话 id）⇒ **回落为"最新登记"的那个，
> 并在返回里带 `pickWhy` 说明原因** —— **绝不静默挑错**。
> 也可看 `web_status → channels.headed.pickWhy` 得知当时的判断依据。
